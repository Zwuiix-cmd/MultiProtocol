...
final class NetItemConverter{
    ...
    public static function convertToProtocol(ItemStackWrapper $source, IRuntimeBlockMapping $blockMapping, MVItemTranslator $itemTranslator = null) : ItemStackWrapper{
        return ItemStackWrapper::legacy(self::convertItemStackToProtocol($source->getItemStack(), $blockMapping, $itemTranslator));
    }

    public static function convertItemStackToProtocol(ItemStack $itemStack, IRuntimeBlockMapping $blockMapping, ?MVItemTranslator $itemTranslator) : ItemStack{
        return self::convertItemToProtocol(TypeConverter::getInstance()->netItemStackToCore($itemStack), $blockMapping, $itemTranslator);
    }

    public static function convertItemToProtocol(Item $item, IRuntimeBlockMapping $blockMapping, ?MVItemTranslator $itemTranslator) : ItemStack{
        if($item->isNull()){
        ...
        $isBlockItem = ($id = $item->getTypeId()) < 256;
        $meta = $item->g();
        ...
        if($isBlockItem){
            ...
            if($block->getTypeId() !== BlockTypeIds::AIR){
                $blockRuntimeId = self::convertBlockRuntimeID(RuntimeBlo::getInstance()->toRuntimeId($block->getFullId()), $blockMapping);
            }
        }
